...
async function init() {
  ...
  function ready() {
    ...
    let currentRnumber;
    const labelOuterRect = new Rectangle(100, 100, "transparent").center().pos(220, 390).drag();
    ...
    function createRandomPic() {
      const pic = new Pic(picData[pictureCount].path).center().mov(0, 200);
      pictureCount++;
    ...
    labelOuterRect.on("pressup", () => {
     ...
     console.log(dragcount);
      if (labelOuterRect.hitTestBounds(abdunatRect) && isAbundant(currentRnumber)) {
        ...
        isabaduntlabel.text = iscorrectnumber.text[lang] + "\n" + `${currentRnumber} ${infoAbdunantnumber.text[lang]}`;
        isCorrect = true;

        //details calculation
        for(let i=0;i<numbers.length;i++){
          if(i === 0){
            isabaduntlabel.text +=`\n${currentRnumber}`+ multiflymassage.text[lang] +`\n${numbers[i]}`;
          }
          else{
            isabaduntlabel.text += `,${numbers[i]}`;
          }
        }
        isabaduntlabel.text += "\n" + multiflicationmassage.text[lang] ;
        console.log(numbers);
        let sum = 0
        for(let i=0;i<numbers.length;i++){
          isabaduntlabel.text += `${numbers[i]}`;
        ...
        isabaduntlabel.text = iscorrectnumber.text[lang] + "\n" + `${currentRnumber} ${infodeficentnumber.text[lang]}`;
        isCorrect = true;
      ...
      if(dragcount >= 20){
        ...
        isabaduntlabel.text = `${attempresult.text[lang]}\n${rightresult.text[lang]}${20 - wrongCount}\n${wrongresult.text[lang]}${wrongCount}\n${percentageofpass.text[lang]} ${Math.round(((20 - wrongCount) / 20) * 100)}%` ;
        S.update();
        return;
      }
      S.update();
