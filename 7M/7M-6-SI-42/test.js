...
async function init() {
  ...
  function ready() {
    ...
    const circleABCbg = new Circle({
      ...
      .center()
      .mov(-150, 250);
    const c = new Circle(10, "transparent")
      .center()
      .mov(
        circleABCbg.x + r * Math.cos(thetaDEF.t1) - fW,
        circleABCbg.y + r * Math.sin(thetaDEF.t1) - fH
      );
    ...
    const c2 = new Circle(15, "transparent")
      .center()
      .mov(
        circleABCbg.x + r * Math.cos(thetaDEF.t2) - fW,
        circleABCbg.y + r * Math.sin(thetaDEF.t2) - fH
      );
    ...
    const c3 = new Circle(15, "transparent")
      .center()
      .mov(
        circleABCbg.x + r * Math.cos(thetaDEF.t3) - fW,
        circleABCbg.y + r * Math.sin(thetaDEF.t3) - fH
      );
    ...
    // Draw triangle ABC
    const triangleAline = new Shape()
      .s(black)
      .ss(2)
      .mt(p1.x, p1.y)
      .lt(p2.x, p2.y)
      .addTo();
    const triangleBline = new Shape()
      .s(black)
      .ss(2)
      .mt(p2.x, p2.y)
      .lt(p3.x, p3.y)
      .addTo();
    const triangleCline = new Shape()
      .s(black)
      .ss(2)
      .mt(p3.x, p3.y)
      .lt(p1.x, p1.y)
    ...
    const leftTriangleShape = new Shape(stage).addTo(stage).drag();
    ...
    leftTriangleShape.on("pressmove", () => {
      console.log(leftTriangleShape.x, leftTriangleShape.y);
      leftCircleD.x = leftTrianglePointD.x;
      leftCircleD.y = leftTrianglePointD.y;
      leftCircleD.mov(leftTriangleShape.x, leftTriangleShape.y);

        leftCircleE.x = leftTrianglePointE.x;
        leftCircleE.y = leftTrianglePointE.y;
        leftCircleE.mov(leftTriangleShape.x, leftTriangleShape.y);

        leftCircleF.x = leftTrianglePointF.x;
        leftCircleF.y = leftTrianglePointF.y;
        leftCircleF.mov(leftTriangleShape.x, leftTriangleShape.y);
      S.update();
    });
    ...
    const texts = [
      ...
      [
        "ABC(<BAC)",
        `${Math.floor(distanceInCm(p1, p2))} cm (AB)`,
        `${Math.floor(distanceInCm(p2, p3))} cm (BC)`,
        `${Math.floor(distanceInCm(p3, p1))} cm (CA)`,
      ],
      ["DEF(<EDF)", "", "", ""],
      ["side ratio", "", "", ""],
    ];
    ...
    function leftUpdateTriangle() {
      const a = Math.floor(
        distanceInCm(leftTrianglePointD, leftTrianglePointE)
      );
      const b = Math.floor(
        distanceInCm(leftTrianglePointD, leftTrianglePointF)
      );
      const c = Math.floor(
        distanceInCm(leftTrianglePointE, leftTrianglePointF)
      );
    ...
    leftCircleD.on("pressmove", (event) => {
      // Updated drag event listener for point D
